...
export default function ParentsForum() {
  ...
  const filtered = posts.filter((p) => {
    if (p.space !== 'parents') return false
    ...
    <div className="fade-in flex flex-col gap-3">
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center py-16">
          ...
          <p className="text-sm font-medium text-[#5D6D7E] mt-3">{t('app.empty.parents_title')}</p>
          <p className="text-xs text-[#9CA3AF]">{t('app.empty.parents_sub')}</p>
        </div>
      ) : (
        filtered.map((post) => (
          <PostCard key={post.id} post={post} space="parents" />
        ))
      )}
    </div>
  )
